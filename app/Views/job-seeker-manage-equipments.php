...
<div class="cover-photo cover-edit"  id="file_image">
                  ...
                  <div class="row bg-white justify-content-center">
                      ...
                      <div class="form-group col-md-12 select-border mb-3 file-img">
                        ...
                        <div class="input-group choose-file" id="filesimages">
                          <input type="file" name="imagefiles" id="fileupload" class="form-control" value=""  accept="image/*" multiple>
                          <label for="imagefiles" class="input-group-text" id="fileuploadfilename">Choose file </label>
                        </div>
                         <span id="file_image_2"></span>
                      ...
                      <div class="form-group col-md-12 select-border mb-3" id="dynamicfield">
                        ...
                        <span id="showthevideo"></span>
                          <div class="thumbnail text-lg-center" id="filevideo2"> 
...
<script>
  function MyProductFunction(id){
    var url= '<?php echo base_url('JobManageEquipments/GetEquipment');?>';
    $.ajax({
                    type: "post",
                    url:  url,
                    data:{id:id},
                       ...
                       if(data['images_name'].length> 0){
                        ...
                        // videos
                         if(data['video_name'].length >0){
                                ...
                                video2+='';   
                                video2 += '<source  src="<?php echo base_url('/public/uploads/Equipmentvideos');?>/'+vals['video']+'" type="video/mp4"/>';
                                video2 += '</video></div>';
                                });
                        ...
                        getstates += '<option value="'+ val['id'] +'">'+ val['name'] +'</option>'; 
                          ...
                          getcity += '<option value="'+ data['city'] +'">'+ data['city_name'] +'</option>';
                            ...
                            var getcounties='';                          
                                ...
                                $('#file_image_2').html('<img src="<?php echo base_url('/public/uploads/equipmentimages');?>/'+data['file_image']+'" /></div>');
                                //$('#file_image_2').html('<div class="thumbnail text-lg-center" id="fileimage2"><img src="<?php echo base_url('/public/uploads/equipmentimages');?>/'+data['file_image']+'" /></div>');
                                ...
                                $.each(data['video_name'], function(key, val){
                                $.each(val, function(key, vals){
                                video2 += '<div><button type="submit" class="close btn btn-danger btn_removes btn_del" onclick=videofunction('+vals['id']+');><span>&times;</span></button><video id="myvideo" controls>';
                                video2+='';   
                                video2 += '<source  src="<?php echo base_url('/public/uploads/Equipmentvideos');?>/'+vals['video']+'" type="video/mp4"/>';
                                video2 += '</video></div>';
                                });
                                $("#filevideo2").html(video2);
                                });
                                }else{
                                $("#filevideo2").html('');   
                                }
...
var formData = new FormData(this);
  var url= '<?php echo base_url('JobManageEquipments/EditEquipment');?>';
  $.ajax({
                    type: "post",
                    url:  url,
                    cache: false,
...
function mydeletfunction(){
  var id  =$('#delid').val();
  var url= '<?php echo base_url('JobManageEquipments/deletedata');?>'
  $.ajax({
                    type: "post",
                    url:  url,
                    data:{id:id},
     ...
     $(document).ready(function(){
      // Check all
        ...
        $('#delete').click(function(){
          var url= '<?php echo base_url('JobManageEquipments/multidelete');?>';
           // Confirm alert
...
<script>
    function imgfunction(id){
         var url= '<?php echo base_url('JobManageEquipments/removeimg');?>';
       $.ajax({
                    url: url,
                    type: 'post',
                    data: {id: id},
...
<!--// video remove from database and folder-->
<script>
function videofunction(id){
    var url= '<?php echo base_url('JobManageEquipments/removevideo');?>';
    $.ajax({
    url: url,
    type: 'post',
    data: {id: id},
