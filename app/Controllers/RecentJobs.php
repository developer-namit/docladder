...
class RecentJobs extends BaseController
{
...
$data=[
 ...
 $searching= new JobseekerSearchModel();
  $pages= $this->request->getVar('pages');
    ...
    $perPage = 9;
    $total   = $searching->count_all($perPage,$page,$data);
    // Call makeLinks() to make pagination links.
    $pager_links = $pager->makeLinks($page, $perPage, $total);
    $start = ($page - 1) * $perPage;
    $data['recentjob']= $searching->RecentSearch($perPage,$start,$data);
   ...
   foreach($result as $key=> $value){
    ...
    $data['margedata']= $result;
      ...
      return view('recent-jobs', $data);
        }  

      // autocomplete skills data  
...
$builder->select('*');
$builder->join('designation', 'designation.designation_id = skills.designation_id');
 
 $builder->like('skills', $name,'after');
