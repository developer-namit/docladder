...
use App\Models\JobEquipmentImg;

class JobManageEquipments extends BaseController
{
       public function index()
    {
      ...
      return view('job-seeker-manage-equipments', $data);
    }
        ...
        $state= new StateModel();
           ...
           if(!empty($cities)){
            ...
            if(!empty($images)){
             ...
             // total count images
             $builder = $db->table('jobseeker_equipment_images')->selectCount('equipment_id');
            $builder->where(['session_id'=>$session_id, 'equipment_id'=>$id]);
            ...
            // multiple videos
            $builder = $db->table('jobseeker_equipment_video')->select('*');
            $builder->where(['session_id'=>$session_id, 'equipment_id'=>$id]);
            $videos= $builder->get()->getResultArray(); 
            if(!empty($videos)){
                $equipment['video_name'][]= $videos;
            }else {
                $equipment['video_name']= 0;
            }
             // total count images
             $builder = $db->table('jobseeker_equipment_video')->selectCount('equipment_id');
            $builder->where(['session_id'=>$session_id, 'equipment_id'=>$id]);
            $videos= $builder->get()->getRowArray(); 
            if(!empty($videos)){
                $equipment['totalvideos']= $videos;
            }else {
                $equipment['totalvideos']= 0;
            }
            ...
            if($videos->isValid() && !$videos->hasMoved()){
             ...
             $model->Updatdata($id,$sessionid,$data);
              ...
              if (!empty($this->request->getFileMultiple('images'))) {
                        ...
                        $uploadimage= New JobEquipmentImg();
                        $builder = $db->table('jobseeker_equipment_images')->selectCount('equipment_id');
              ...
              if (!empty($this->request->getFileMultiple('video_name'))) {
                        ...
                        $save = $db->table('jobseeker_equipment_video')->insert($video);
                        $msg = 'Files have been successfully uploaded';
                        }
...
public function removeimg(){
       ...
       $session_id= $session->get('id');
       $id= $this->request->getVar('id');
       $build= New JobEquipmentImg();
        $build->where(['session_id'=> $session_id, 'id'=> $id]);
        $img=$build->first();        
         $images=$img['images'];
        unlink(FCPATH.'public/uploads/Equipmentimages'.'/'.$images);
         $model= New JobEquipmentImg();
           $model->where(['session_id'=> $session_id, 'id'=> $id]);
          $del= $model->delete();
...
// video remove data from database 
...
public function removevideo(){
     $db      = \Config\Database::connect();
       ...
       $session_id= $session->get('id');
       $id= $this->request->getVar('id');
       $build= $db->table('jobseeker_equipment_video');
        $build->where(['session_id'=> $session_id, 'id'=> $id]);
        $video=$build->get()->getRowArray();        
         $videos=$video['video'];
        unlink(FCPATH.'public/uploads/Equipmentvideos'.'/'.$videos);
         $model= $db->table('jobseeker_equipment_video');
           $model->where(['session_id'=> $session_id, 'id'=> $id]);
          $del= $model->delete();
